ms です。TocTitle︰ 探索サービスの 停留 API リファレンス タイトル︰ 探索サービスの 停留 API リファレンスの説明︰ Office 365 の探索サービス API を使用して、Office 365 アプリケーションが接続できるサービスにアクセスするためのエンドポイントを自動的に検索する方法を参照します。ms です。ContentId: 799c9512-7020-4033-b7b8-1e9a2174555d ms.topic: ms.date を参照 (API): 2015 年 6 月 9 日
...
これにより、クライアントの 网站 ページにユーザーがアカウント情報を入力します。探索サービスを続行するために必要なエンドポイントを返します。ユーザーには、アプリケーションがしようとすると、最初に使用されます。アプリケーションを示します。
...
|`lcid`|string| 省略可能です。電子メール HRD の UI をローカライズするのには 10 進数の LCID です。<br>例︰ 1031年 </br><br>**メモ**この API 意図的にしないために、受信ユーザーの電子メールの現在のドメインからユーザーの電子メールを送信することによってユーザーのプライバシーを侵害する可能性があります。</br>|
...
|&amp;account_type =。 | 1 Microsoft アカウント （生存） <br>2-組織のアカウント (Office 365)</br>|
|&amp;authorization_service =。 | エンドポイントの URL は、クライアントが認証コードを取得できます。|
|&amp;token_service =。 |エンドポイントの は、クライアントがアクセス URL トークン、更新トークンの認証コードを交換できます。|
|&amp;スコープ =。 | 元のスコープは、ターゲットとなる領域に変換されます。のみ、クライアントは、Office 365 の範囲の用語を知っておく必要があります。対象となる領域が 活动 の場合は、元の Office 365 のスコープは、ライブの用語に変換されます。|
...
|&amp;discovery_service =。 |エンドポイントの は、クライアントがターゲット URL サービスを検出できます。|
|&amp;discovery_resource =。 |探索サービスのリソース id です。探索サービスのトークン要求の一部としてトークン サービスに渡されます必要があります。|

**メモ**このすべての情報は、このユーザー アカウントで静的です。したがってクライアントが、キャッシュして不要な 用户界面 を使用してユーザーを迷惑なを避けるために再利用します。
