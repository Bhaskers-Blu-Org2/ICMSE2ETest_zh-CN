...
.NET または JavaScript クライアント ライブラリを使用して Outlook の 停留 API にアクセスするには、アクセス トークンを取得し、Outlook のサービス クライアントを取得する必要があります。次に、予定表のデータと対話するのには、非同期クエリを送信できます。
